...
PYEOF
  ...
  # ── hyprsaver (screensaver palette locked to the theme's accent/dim
  # pair) — see [[oled-idle-off]]. hyprsaver is spawned fresh by hypridle
  # on every idle cycle (killed via `--quit` on resume, not kept running),
  # so it always reads this file from scratch — no live-reload needed,
  # unlike waybar below. Two-color cosine-gradient trick (a=midpoint,
  # b=half-difference, c=[1,1,1], d=[0,0,0]) makes the shader's whole
  # color range oscillate between DIM and ACC instead of the default
  # rainbow cycle — see hyprsaver's own config.toml comments for the
  # general a/b/c/d formula if this ever needs adjusting.
  mkdir -p "$HOME/.config/hypr"
  hs_palette=$(python3 - "$ACC" "$DIM" <<'PYEOF'
import sys
c1 = [int(sys.argv[1][i:i+2], 16) / 255 for i in (1, 3, 5)]
c0 = [int(sys.argv[2][i:i+2], 16) / 255 for i in (1, 3, 5)]
a = [(x + y) / 2 for x, y in zip(c0, c1)]
b = [(y - x) / 2 for x, y in zip(c0, c1)]
fmt = lambda v: "[%.4f, %.4f, %.4f]" % tuple(v)
print(f"a = {fmt(a)}\nb = {fmt(b)}")
PYEOF
)
  cat > "$HOME/.config/hypr/hyprsaver.toml" <<EOF
# generated by theme.sh — palette: $name
[general]
shader = "cycle"
palette = "theme"
